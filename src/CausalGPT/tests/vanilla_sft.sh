...
if [ "$(uname -s)" = "Darwin" ] && [ "${RUN_FULL:-0}" != "1" ]; then
    ...
    python -m scripts.chat_sft --help >/dev/null
...
torchrun --standalone --nproc_per_node=$NPROC_PER_NODE -m scripts.chat_sft -- --run=$WANDB_RUN --device_batch_size=8 --num_epochs=2
